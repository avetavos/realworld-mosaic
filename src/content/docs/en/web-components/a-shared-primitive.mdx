...
The design-system package's first file — just enough for `@mosaic/design-system/m-price` to resolve as a workspace package. [Tokens and Primitives](/mosaic/en/design-system/tokens-and-primitives/) later extends this same file with the `.` main export and `./tokens.css`.
...
### 2. `packages/design-system/src/m-price.ts`
...
### 3. Consume it from React (the catalog)
...
Teach JSX about the tag once, in the catalog's own `custom-elements.d.ts` (the same pattern — and the same `export {}` requirement — as [the catalog-app declaration](/mosaic/en/web-components/custom-element-boundary/)):
...
### 4. Consume it from Svelte and Astro
...
Then confirm a real consumer picks it up. The design-system package has no build step of its own (Vite reads its `.ts` source directly) — the real proof is that the catalog, which now imports `@mosaic/design-system/m-price`, still builds:

```bash
pnpm --filter catalog build
# builds; the catalog's product cards render through the shared primitive.
```

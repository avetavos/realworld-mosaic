...
ไฟล์แรกของ design-system package — แค่พอให้ `@mosaic/design-system/m-price` resolve ได้ในฐานะ workspace package ทีหลัง [Tokens and Primitives](/mosaic/th/design-system/tokens-and-primitives/) จะต่อยอดไฟล์เดียวกันนี้ด้วย `.` main export กับ `./tokens.css`
...
สอน JSX ให้รู้จัก tag ครั้งเดียว ใน `custom-elements.d.ts` ของ catalog เอง (pattern เดียวกัน — และต้องมี `export {}` เหมือนกัน — กับ[การ declare catalog-app](/mosaic/th/web-components/custom-element-boundary/)):
...
จากนั้นยืนยันว่า consumer จริงหยิบไปใช้ได้ design-system package ไม่มี build step ของตัวเอง (Vite อ่าน `.ts` source ตรง ๆ) — บทพิสูจน์จริงคือ catalog ที่ตอนนี้ import `@mosaic/design-system/m-price` ยัง build ผ่าน:

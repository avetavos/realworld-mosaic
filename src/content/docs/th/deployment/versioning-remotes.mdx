...
ผลตอบแทนที่สถาปัตยกรรมทั้งหมดสร้างมาเพื่อไปให้ถึง: การ ship version ใหม่ของ *หนึ่ง* remote แบบ live โดยไม่ต้อง rebuild หรือ redeploy shell หรือ remote ตัวอื่นใด เราจะ version `remoteEntry` ของ catalog, roll out มัน, และดู shell หยิบมันไปใช้ตอนผู้เข้าชมโหลดหน้าครั้งถัดไป — คำสัญญา "ship ทีมเดียว" ที่พิสูจน์ตั้งแต่ต้นจนจบ

## Why

เพราะ shell [โหลด remote ตอน runtime จาก URL ของ `remoteEntry`](/mosaic/th/federation/) version ของ remote จึงอยู่ที่ฝั่งของ remote ทั้งหมด เมื่อ [catalog team deploy](/mosaic/th/deployment/deploy-each-slice/) build ใหม่ พวกเขา publish `remoteEntry.js` ใหม่ที่ URL คงที่ที่ shell รู้จักอยู่แล้ว shell ไม่ได้ import catalog ตอน build ดังนั้นมันไม่มีอะไรต้อง rebuild — *ครั้งถัดไป* ที่ browser โหลด shell มันจะ fetch `remoteEntry.js` ปัจจุบันแล้วได้ catalog ใหม่ นั่นคือ release train ที่ถูกแยกออกจากกัน: catalog team ship ตามตารางของตัวเอง, cart team ไม่รู้สึกอะไรเลย, shell ไม่ถูกแตะ

รายละเอียดที่ทำให้สิ่งนี้ปลอดภัย:

- **Cache-busting the entry.** `remoteEntry.js` คือ manifest ที่ shell อ่านเพื่อหา chunk ปัจจุบัน; มันต้องถูก serve ด้วย header แบบ short/no-cache เพื่อให้ deploy ใหม่ถูกเห็นทันที ขณะที่ *content* chunk ที่มันชี้ไปเป็น immutable และ hash ไว้เพื่อ cache lifetime ที่ยาว ทำกลับกันแล้วผู้เข้าชมจะโหลด catalog ตัวเก่าจาก cache ต่อไปเรื่อย ๆ
...
เปลี่ยนแค่ catalog, build ด้วย build id สด, deploy asset ของมัน ไม่มีอะไรของ shell หรือ cart ถูกแตะ
...
ถ้าคุณอยากเลือก URL ของ remote — หรือ pin path ที่ version ไว้อย่าง `/v2/remoteEntry.js` — ทั้งหมดตอน runtime แทนที่จะตอน shell build ให้ register มันด้วย runtime API สิ่งนี้ทำให้ shell adopt remote ใหม่หรือ canary URL ได้โดยไม่ต้อง rebuild เลย
...
คุณ ship ทีมเดียวได้โดยไม่ต้อง rebuild ตัวอื่น คุณสมบัติเดียวนั้นคือสิ่งที่ดีไซน์ shell-loads-remotes-at-runtime, Web-Component boundary, per-remote BFF, และ event bus ที่ decouple กัน ทั้งหมดมีอยู่เพื่อทำให้มันปลอดภัย
...
3. การ version *code ของ remote เอง* นั้นฟรี แต่การเปลี่ยน major ของ *shared* dependency ไม่ฟรี Module Federation ทำอะไรตอนโหลดเพื่อรักษาความปลอดภัยตรงนั้น และมันมีต้นทุนอะไรเทียบกับ single-build monolith?
4. `registerRemotes()` ตอน runtime ให้คุณทำอะไรได้ที่การตั้ง `entry` ใน `vite.config.ts` ของ shell ทำไม่ได้?

## Recap

คุณ version remote แล้ว roll out มัน: `remoteEntry.js` ใหม่ที่ publish ที่ URL คงที่, ถูก shell หยิบไปใช้ตอนโหลดครั้งถัดไป, พร้อม manifest ที่ stamp ไว้เพื่อพิสูจน์ว่า build ไหน live — และ shell กับ cart ไม่เคย rebuild นั่นคือผลตอบแทน "ship ทีมเดียว" ที่พิสูจน์ตั้งแต่ต้นจนจบ พร้อมรายละเอียดเรื่อง cache และ shared-version ที่รักษาความปลอดภัย

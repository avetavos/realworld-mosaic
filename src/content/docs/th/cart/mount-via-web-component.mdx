...
description: "compile Svelte cart เป็น <cart-app> custom element, expose ./register ผ่าน Module Federation, แล้ว mount มันใน React shell — ผลตอบแทนของ cross-framework"
sidebar:
  order: 2
---
...
แต่ browser มี framework-neutral component model อยู่แล้ว: **custom elements** ทุก framework render tag `<cart-app>` ได้ และ browser instantiate มัน ดังนั้นแทนที่จะ expose *Svelte component* cart จึง expose *registrar* — module ที่มีหน้าที่เดียวคือเรียก `customElements.define('cart-app', …)` Svelte 5 ทำ definition นั้นให้เราเมื่อเราตั้ง [`customElement` compiler option](https://svelte.dev/docs/svelte/custom-elements) แล้วให้ component มี tag `<svelte:options customElement="cart-app" />` การ import module รัน definition ตั้งแต่นั้น `<cart-app>` เป็น HTML element ของจริงที่ shell render, style, และลบได้

นี่คือการ generalise ที่ [Module 5](/mosaic/th/web-components/) สัญญาไว้: **custom element คือ universal mount boundary** catalog *ก็* expose มันได้ ส่วน cart *ต้อง* เพราะมันเป็น framework คนละตัว รอยต่อเดียวกัน และทุก remote ในอนาคตก็ใช้มันได้

## Pros & cons

**Exposing a custom-element registrar vs. exposing a framework component**

- **Pros:** framework-agnostic — host render tag ไม่ใช่ type ของ React/Svelte/Vue remote เปลี่ยน framework ภายในของมันได้โดยไม่พัง contract ของ host รูปแบบ (`import for side effect` แล้ว render tag) เหมือนกันเป๊ะสำหรับทุก remote ที่ไม่ใช่ React
- **Cons:** คุณเสีย typed props/children ที่ React component ให้ — การสื่อสารเกิดผ่าน attribute, property, และ event แทน Svelte remote ship runtime ของตัวเอง (ไม่มี singleton ที่ share กัน) การ style ด้วย shadow-DOM มีกฎของมันเองให้เรียนรู้
...
สองการเปลี่ยนจาก config บทที่แล้ว: เปิด custom-element compilation แล้วเพิ่ม federation plugin ที่ expose `./register` ตาม build contract cart share **ไม่มีอะไรเลย** กับ host — มันเป็น Svelte จึงไม่มี React singleton ให้ใช้ซ้ำ — และ `type: 'module'` กับ `build.target: 'esnext'` จำเป็นสำหรับ Vite remote
...
host ลิสต์ `catalog` อยู่แล้ว เพิ่ม `cart` เข้าไปข้าง ๆ สังเกตว่า entry ของ cart หน้าตาเหมือน React remote เป๊ะ — host ไม่รู้และไม่สนว่ามันเป็น Svelte
...
เรา import registrar เพื่อ side effect ของมัน แล้ว render tag เรา define element หนึ่งครั้งตอน mount แล้ว track ว่ามันพร้อมเมื่อไหร่ เพื่อไม่ให้ React render `<cart-app>` ก่อนที่มันจะมีอยู่ JSX ส่ง `cart-app` ตรงไปที่ DOM — มันเป็น custom element ของจริง ดังนั้น React แค่สร้างมัน
...
TypeScript ไม่รู้จัก `<cart-app>` โดย default declare มันหนึ่งครั้งเพื่อให้ JSX type-check ผ่าน:
...
- ใน DevTools มี `<cart-app>` element ของจริงใน DOM (กางมันดูจะเห็น shadow DOM ของ Svelte)
...
2. `register.ts` ไม่มี export — มีแค่ import เดียว มันกำลังทำอะไรจริง ๆ และทำไมแค่นั้นถึงพอ?
3. federation config ของ cart ตั้ง `shared: []` ในขณะที่ catalog share `react`/`react-dom` ทำไม empty ถึงถูกต้องสำหรับ cart?

...
TypeScript ไม่รู้จัก `<cart-app>` โดย default เพิ่มเข้าไปใน `custom-elements.d.ts` ของ shell จาก [Web Components Interop](/mosaic/th/web-components/custom-element-boundary/) — ไฟล์เดียวกัน เพิ่มอีก entry บน `IntrinsicElements` ที่ต่อยอดไว้แล้ว:

...
behaviour ที่ถูกต้องสำหรับ **cold deep link** และสำหรับปุ่ม **Back** มีคน paste `https://mosaic.app/catalog/p/42` ลง tab ใหม่ หรือ email การตลาด link ตรงไปที่มัน ยังไม่มีอะไรโหลดเลย — shell ยังไม่รัน, catalog remote ยังไม่ถูก fetch เราต้องการให้ URL นั้น boot shell, lazy-load catalog remote แล้ว land ที่ product-detail view โดย browser history ครบถ้วนเพื่อให้ Back พาผู้ใช้กลับไปที่ที่เขามา
...
3. พอ chunk ของ remote มาถึง descendant `<Routes>` ของมัน match ส่วนที่เหลือแบบ *relative* `p/42` แล้ว render detail view

เพราะทั้งหมดนี้รันกับ history object **ตัวเดียว** stack Back/forward ของ browser จึง coherent ฟรี — ทุก `<Link>` และ `useNavigate` ไม่ว่าใน shell หรือ remote push ลง stack เดียวกัน ปัญหา synchronisation ที่คนกลัวกับ "router สองตัว" จะโผล่ก็ต่อเมื่อคุณใส่ history ตัวที่สองเข้าไป; design แบบ descendant-routes ไม่เคยทำแบบนั้น การให้ router ของ shell กับ remote sync กันจึงไม่ใช่การต่อสายเพิ่ม — มันคือ *การไม่มี* router ตัวที่สอง

failure mode ที่ต้องวางแผนรับคือ step 2: `remoteEntry.js` ของ remote อาจโหลดไม่สำเร็จ (deploy กำลังวิ่งอยู่, network สะดุด) deep link ที่ fetch remote ไม่ได้ต้อง degrade อย่าง graceful ซึ่งเป็นเหตุผลที่บทนี้ส่งต่อไปยัง [Resilience & Performance →](/mosaic/th/resilience/)
...
route ของ remote อ่าน `:id` จาก shared router ตอน cold load มันรันทันทีที่ chunk ของ remote เสร็จ — param อยู่ใน URL มาตลอดอยู่แล้ว
...
Svelte cart เรียก `useNavigate` ไม่ได้ แต่มัน share `window.history` ตัวเดียวกัน เพื่อสะท้อน view ภายในลงใน URL — และเพื่อ react เมื่อผู้ใช้กด Back — มันใช้ platform ตรง ๆ:
...
เพราะยังมี history เดียว react-router ของ shell กับ `popstate` listener ของ cart จึงไม่มีวันขัดกัน — พวกมันสังเกต object เดียวกันจากคนละฝั่ง
...
- deep-link ไปที่ `/cart/checkout` แล้วกด Back: `popstate` listener ของ cart พามันกลับไปที่ cart view sync กับ shell

ยืนยันว่า production build serve deep link ได้ (SPA fallback คือสิ่งที่ทำให้มันจริง ไม่ใช่แค่ dev server):
...
2. ทำไม Back/forward ถึง "ทำงานเอง" โดยไม่มี synchronisation code — property อะไรของ design ที่รับประกันมัน?
3. server-side rewrite ไป `index.html` ป้องกันอะไร และทำไม deep link คือเคสที่เปิดโปงการไม่มีมัน?
4. Svelte cart ใช้ `history.pushState` และ `popstate` listener แทน react-router ทำไมมันยัง sync กับ router ของ shell ได้เป๊ะ ๆ?
...
การ resolve remote บน deep link สมมติว่า remote โหลดได้จริง ซึ่งไม่เสมอไป — `remoteEntry.js` อาจ fail กลาง deploy ต่อไปเราทำให้มันปลอดภัย: [Resilience & Performance →](/mosaic/th/resilience/)

...
**shell** เป็น React app ที่เป็นเจ้าของทุกอย่างที่แชร์ร่วมกัน: layout, top-nav, session และ route ระดับบน มัน **ไม่** import remote ตอน build time — แต่โหลดพวกมันตอน **runtime** จาก URL ของ `remoteEntry` แต่ละตัว ดังนั้นทีม catalog จะ deploy catalog ตัวใหม่ได้ แล้ว shell จะรับมันมาใช้ในการโหลดครั้งถัดไป โดยไม่ต้อง rebuild ทั้ง shell และ remote ตัวอื่น คุณสมบัติเดียวนั้น — **independent deployment** — คือสิ่งที่ทุกอย่างที่เหลือในสถาปัตยกรรมนี้มีอยู่เพื่อทำให้มันปลอดภัย
...
- **Vertical ownership** remote แต่ละตัวเป็นเจ้าของ **UI และ data ของมัน** (Hono BFF บาง ๆ) slice จึงเป็น feature เต็ม ๆ ที่ทีมเดียวเป็นเจ้าของได้ตั้งแต่ต้นจนจบ
...
- **Content remote** — **Astro** (SSR/static) Astro ไม่ runtime-federate แบบ SPA มันจึงถูกผนวกเข้ามาผ่านเส้นทาง Web-Component / SSR-composition — เป็นบทเรียนที่ตั้งใจให้เห็นข้อจำกัดของ Module Federation
...
- ทำไม shell ถึงโหลด remote ตอน *runtime* จาก `remoteEntry` แทนที่จะ import มันตอน build time?
- Astro เป็น Module Federation remote แบบที่ React และ Svelte เป็นไม่ได้ ทำไม — และ Mosaic ผนวกมันเข้ามาได้อย่างไรถึงอย่างนั้น?

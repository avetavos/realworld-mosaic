...
บทนี้สร้าง package และ primitive สามตัวของมัน เราเจอ pattern custom-element มาแล้วใน [Web Components Interop →](/mosaic/th/web-components/); ตรงนี้เราใช้มันอย่างตั้งใจเป็น shared-UI layer
...
สอง feature ของ web platform ทำให้มันทำงานร่วมกันได้:
...
การแบ่งจึงตั้งใจ token เปิด (global, themeable, override ต่อ surface ได้); ภายในของ primitive ปิด (encapsulate ใน shadow DOM เพื่อไม่ให้ stylesheet ของ remote ตัวไหนรั่วเข้ามาทำมันพัง)

## Pros & cons

**Tokens as CSS custom properties vs. tokens as a JS/TS object**

- **Pros:** CSS variable เป็น live และ cascade — set `--m-color-primary` บน wrapper แล้วทุกอย่างข้างใต้ re-theme โดยไม่ต้องมี JS มัน cross shadow boundary ฟรี shadow-DOM primitive จึงอ่านมันได้โดยไม่ต้องต่อสายอะไร
...
2. อะไรพังถ้า remote สองตัวต่างเรียก `customElements.define("m-button", ...)` และ guard ใน `index.ts` ป้องกันมันยังไง?
3. ทำไม token จึงตั้งใจให้ *global* (เปิดให้ override) ในขณะที่ internal style ของ primitive ตั้งใจให้ *encapsulate* (shadow DOM)?

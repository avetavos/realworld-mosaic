...
- **Pros:** มีที่เดียวเป็นเจ้าของ layout, nav, session และ routing user จึงเห็น app เดียวที่กลมกลืน; remote โฟกัสอยู่กับ slice ของมัน; มี entry point เดียวชัด ๆ ให้โหลด
- **Cons:** host เป็น shared dependency ที่ทุกทีมพึ่งพา — เปลี่ยนกรอบทีกระทบทุกคน จึงต้องมี ownership ที่รอบคอบ และมันกลายเป็นคอขวดได้ถ้าโตเกิน "กรอบ" การรักษาให้มันบางคือวินัย
...
shell เป็น React + Vite app มันรันที่ port 5000 และตอนนี้พึ่งแค่ React กับ Vite React plugin
...
เปิด `http://localhost:5000/` คุณควรเห็น header **🧩 Mosaic**, nav link สามอัน และ placeholder "The shell is running" ใน main slot จากนั้นยืนยันว่ามัน build สะอาด — check ที่ทุกบทจบด้วย:
...
1. shell เป็นเจ้าของอะไรที่ remote ไม่เป็น และทำไมต้องมี app เดียวเป๊ะ ๆ เป็นเจ้าของมัน?
...
shell รันแล้ว: React + Vite app บน port 5000 ที่มี header, top-nav และ mount slot `<main>` รอ remote อยู่ ตอนนี้มันเป็น app ธรรมดา — ซึ่งคือประเด็น federation คือ layer ถัดไป ไม่ใช่รากฐาน

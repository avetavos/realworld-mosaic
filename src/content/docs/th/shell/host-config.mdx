...
ตัว catalog remote เองยังไม่มีอยู่ — เราจะสร้างใน [Module 4](/mosaic/th/catalog/) สิ่งที่เราสร้างตรงนี้คือ *ฝั่ง host* ของ contract: config และโค้ดโหลด พร้อมให้ remote จริงมาทำให้ครบ นี่คือ config ที่ทุก module ถัดไปพึ่งว่าต้องถูกต้องเป๊ะ เราจึงทำตามอย่างละเอียด
...
- **Cons:** remote คือ network dependency: ช้าได้, version ตามหลังได้ หรือโหลดไม่ขึ้นเลยก็ได้ host ต้องจัดการ latency (`<Suspense>`) และความล้มเหลว (error boundary ใน [Module 12](/mosaic/th/resilience/)) — ปัญหาที่ build-time import ไม่เคยมี
...
พอคุณรัน `pnpm --filter shell dev` ตอนนี้ shell โหลดขึ้นและ *พยายาม* fetch `http://localhost:5001/remoteEntry.js` จนกว่า catalog remote จะรัน request นั้นจะ fail ใน console — คาดไว้แล้ว และเป็น failure mode ที่ [Module 12](/mosaic/th/resilience/) สอนให้คุณจัดการเป๊ะ ๆ host ถูกต้อง; แค่รอ remote อยู่

...
### 1. `apps/catalog/bff/src/products.ts`
...
ต่อยอด `app.ts` ที่มีแค่ health check จาก [The BFF stack](/mosaic/th/setup/the-bff-stack/) ด้วยสอง route ของ product `server.ts` ไม่ต้องแก้ — มัน import `app` นี้แล้ว serve บน 4001 อยู่แล้ว
...
// The health check every BFF exposes — unchanged from the setup lesson.
...
pnpm --filter @mosaic/catalog-bff dev    # BFF on 4001

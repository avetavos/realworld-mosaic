...
package `@mosaic/session`: store เดียวที่ใช้ร่วมกันสำหรับ "ใคร sign in อยู่" มันเป็น mock (ชื่ออะไรก็ sign in ได้ state อยู่ใน `localStorage` ไม่มี auth จริง) พร้อม API เล็ก ๆ ที่ตั้งใจออกแบบ:
...
ส่วนสำคัญไม่ใช่ตัวโค้ด — แต่คือการที่ package นี้เป็น **Module Federation singleton** micro-frontend ทุกตัว import instance *ตัวเดียวกัน* จึงมี session เดียวเป๊ะ ๆ บนหน้าเว็บ sign in ครั้งเดียวแล้วทั้งแอปเห็นตรงกัน มันยังประกาศทุกการเปลี่ยนแปลงบน [`@mosaic/bus`](/mosaic/th/communication/the-event-bus/) เป็น `auth:changed` เพื่อให้แม้แต่โค้ดที่ไม่ import session ก็ react ได้

## Why

identity คือ state ชิ้นเดียวที่กินขอบเขตทุก remote จริง ๆ catalog อาจทักคุณด้วยชื่อ, cart ต้องรู้ว่า cart นี้ของใคร, shell แสดงปุ่ม sign-in/out ใน SPA เดี่ยวนี่คือ context provider ตัวเดียว แต่ข้าม remote ที่ deploy อิสระมันเป็นกับดัก: ถ้า remote แต่ละตัวเก็บ copy ของ "current user" ของตัวเอง พวกมันจะ drift — shell คิดว่าคุณคือ Ada แต่ cart ยังคิดว่าคุณเป็น guest
...
เราตั้งใจให้มันเป็น mock auth จริง (token, refresh, httpOnly cookie) เป็นหัวข้อใหญ่ที่จะกลบบทเรียนตัวจริง ซึ่งคือ *การ share state ข้าม MFE* `localStorage` ให้ persistence ข้าม reload โดยไม่ต้องมี backend และการสลับไปใช้ provider จริงทีหลังก็ไม่เปลี่ยนรูปทรงของ shared singleton
...
- **Cons:** ทุก remote ตอนนี้ share runtime dependency และต้องเห็นตรงกันเรื่อง version (singleton mismatch จะ warn หรือ break) session กลายเป็นจุด coordination — breaking change ต่อ API ของมันกระเพื่อมไปทุก consumer
...
- **Cons:** มันไม่ secure และไม่จริง — `localStorage` อ่านได้โดย script ตัวไหนก็ได้ และไม่มี server verify อะไรเลย มันเป็นตัวแทนของ identity ไม่ใช่ authentication; production ต้องมี provider จริงหลัง interface เดียวกัน

## Set it up

### 1. `packages/session/package.json`

พึ่ง bus เพราะมันประกาศ `auth:changed`
...
เพิ่ม `@mosaic/session` — คู่กับ bus ที่มันพึ่ง — เข้าไปใน `shared` ทั้งใน host **และ remote ทุกตัว** เป็น singleton:
...
จากนั้นลองใช้มันใน browser เริ่ม shell (`pnpm --filter shell dev`) เปิด console:
...
ตอนนี้ **reload หน้า** แล้วรัน `(await import('@mosaic/session')).getSession()` — มันยัง return Ada พิสูจน์ persistence ของ `localStorage` จากนั้น:
...
3. ทำไม `localStorage` เป็น mock ที่รับได้ในที่นี้แต่ไม่ใช่ authentication จริง? อะไรที่ยังคงเดิมเมื่อคุณแทนมันด้วย provider จริง?
4. `getSession()` return แบบ synchronous ในขณะที่ fetch `/me` จริงจะเป็น async singleton ให้อะไรกับคุณที่ทำให้การอ่านแบบ synchronous ถูกต้อง?

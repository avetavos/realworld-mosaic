...
[บทที่แล้ว](/mosaic/th/auth-state/session-singleton/) สร้าง session เดียวที่ใช้ร่วมกัน บทนี้ว่าด้วยการ **react** ต่อมันให้ถูกต้องข้าม framework failure mode ที่ต้องเลี่ยง: remote อ่าน `getSession()` ครั้งเดียวตอน mount แล้วไม่อัปเดตอีก คุณ sign in แล้ว catalog ยังทักคุณว่า "guest" จนกว่าจะ reload shared state มีประโยชน์ก็ต่อเมื่อ consumer *subscribe* การเปลี่ยนแปลง ไม่ใช่แค่อ่านครั้งเดียว

เพราะ session เป็น singleton remote จึง import มันตรง ๆ และเรียก `subscribe()` ได้ — callback fire จาก store ตัวเดียวกับที่ shell เขียน นั่นคือเส้นทางที่ง่ายที่สุดและ typed เต็ม (คุณได้ `Session` ไม่ใช่ event payload ที่ไม่มี type) แต่มัน *couple* remote นั้นกับ `@mosaic/session` consumer บางตัวไม่ควรรับ dependency นั้น — design-system web component หรือ Astro content ที่ไม่ได้เป็น Module Federation remote เลย สำหรับพวกนั้น `auth:changed` บน bus พาข่าวเดียวกันมาโดยไม่ต้อง import การเลือกระหว่างสองทางนี้คือบทเรียนตัวจริง: **import-and-subscribe เมื่อคุณพึ่ง session อยู่แล้ว; ฟังบน bus เมื่อคุณไม่อยากพึ่ง**
...
- **Cons of `subscribe()`:** couple consumer กับ `@mosaic/session` — มันต้อง share singleton และตาม version `bus.on` เลี่ยง dependency ทั้งหมด (เหมาะกับ Astro island หรือ element ที่ไม่ผูก framework) แลกกับ payload ที่บางกว่าและ typing ที่หลวมกว่า
...
- **Cons of reacting:** ทุก consumer ต้องมี subscription และการ cleanup ของมัน (effect, `onMount` teardown) การพลาด unsubscribe ทำให้ listener รั่ว; การลืม react ไปเลยทำให้ UI ค้างที่ดูปกติจนกว่าจะมีใคร sign in

## Set it up

### 1. `apps/shell/src/AuthMenu.tsx`

shell คือที่ที่คุณ sign in และ out มันอ่าน session แล้ว react เหมือน consumer ตัวอื่น ๆ
...
catalog อยู่ใน shared module graph อยู่แล้ว มันจึง import session และ subscribe `useEffect` return unsubscribe ของ `subscribe` เป็น cleanup
...
Svelte cart อ่าน singleton ตัวเดียวกัน `$state` เก็บ session; `onMount` ต่อ `subscribe` และ return teardown ของมัน
...
design-system element หรือ Astro content island react ได้โดยไม่ต้องพึ่ง `@mosaic/session` — มันต้องการแค่ bus:
...
เพื่อดูเส้นทาง bus เพิ่ม snippet ของ step 4 ไปที่ element `.who` แล้วยืนยันว่ามันอัปเดตเมื่อ `auth:changed` โดยไม่ import session
...
3. catalog (React) และ cart (Svelte) ต่างแสดงชื่อใหม่โดยไม่ reload ข้อเท็จจริงเดียวเกี่ยวกับ `@mosaic/session` อะไรที่ทำให้มันได้ผลข้ามสอง framework?
4. ทำไมเส้นทาง bus พก `{ userId, name }` แทนที่จะเป็น `Session` เต็ม — และ consumer เสียอะไรไปเมื่อใช้มัน?

...
[React catalog](/mosaic/th/catalog/the-catalog-remote/) emit `cart:add` เมื่อคุณคลิก "Add to cart" [Svelte cart](/mosaic/th/cart/mount-via-web-component/) ได้ยินมัน อัปเดต state ของตัวเอง แล้ว emit `cart:changed` shell ได้ยิน `cart:changed` แล้วอัปเดต badge ใน top-nav สาม hop, bus เดียว, ไม่มี import ข้าม remote เลย

## Why

นี่คือช่วงที่ architecture คุ้มกับความซับซ้อนของมัน SPA เดี่ยว ๆ แค่เรียก hook `useCart()` หรืออ่าน shared store — framework เดียว module graph เดียว ง่ายมาก Mosaic ทำแบบนั้นไม่ได้: emitter เป็น React, receiver เป็น Svelte และทั้งคู่ ship แยกกัน สิ่งเดียวที่ทั้งสามใช้ร่วมคือ [`@mosaic/bus`](/mosaic/th/communication/the-event-bus/) และ bus ก็ไม่รู้จักตัวไหนเลย

สังเกตรูปทรงของ data flow catalog **ไม่ได้** อัปเดต cart ตรง ๆ และ **ไม่ได้** อัปเดต badge — มันประกาศข้อเท็จจริง ("product นี้ถูกเพิ่ม") แล้วหยุด cart เป็นเจ้าของ cart state มันจึงเป็นตัวที่คำนวณ total ใหม่แล้วประกาศ state ใหม่ (`cart:changed`) shell เป็นเจ้าของ badge มันจึงเป็นตัวที่ฟัง `cart:changed` MFE แต่ละตัวเป็นเจ้าของ state slice ของตัวเองและแค่ *broadcast* การเปลี่ยนแปลง วินัยนั้น — เป็นเจ้าของ state ของตัวเอง, emit ข้อเท็จจริง, react ต่อข้อเท็จจริงของคนอื่น — คือสิ่งที่ทำให้รอยต่อ decoupled อยู่ได้เมื่อแอปโตขึ้น
...
- **Pros:** emitter ไม่ block หรือรอ และ receiver อิสระที่จะ ignore, batch หรือ debounce cart ยังเป็นเจ้าของความจริงของ cart แต่เพียงผู้เดียว คนอื่น react ต่อสิ่งที่มันประกาศ
- **Cons:** catalog ไม่ได้ยืนยันตรง ๆ ว่าการ add สำเร็จ — ถ้าอยากได้ feedback (เช่นตัวเลข "1 in cart") มันต้อง *ฟัง* `cart:changed` ด้วย สอง event จำลองหนึ่ง interaction ซึ่งคุณต้องจำไว้ในหัว

## Set it up

### 1. `apps/catalog/src/ProductCard.tsx`

React catalog emit `cart:add` มัน import bus — ไม่เคย import cart
...
Svelte cart เป็นเจ้าของ cart state มันฟัง `cart:add` อัปเดต list ของตัวเอง แล้ว emit `cart:changed` พร้อม total ใหม่ Svelte 5 rune (`$state`) ให้ reactivity แบบลึก การ push เข้า array จึงอัปเดต UI `onMount` return unsubscribe ของ bus ไว้ cleanup
...
shell เป็นเจ้าของ badge ใน top-nav มันฟัง `cart:changed` แล้ว render count `useEffect` return unsubscribe ของ `bus.on(...)` ตรง ๆ เป็น cleanup
...
3. ถ้าคุณอยากให้ปุ่มของ catalog แสดง "1 in cart" หลัง add มันจะ subscribe event ไหน และทำไมมันอ่าน state ของ cart ตรง ๆ ไม่ได้?
4. อะไรในการต่อสายนี้ที่ยังให้ทีม cart เปลี่ยนวิธีที่ cart เก็บ line ของมันได้ โดยไม่ทำ catalog หรือ shell พัง?

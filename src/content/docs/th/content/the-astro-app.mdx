...
ไม่ใช่ทุกส่วนของ storefront ที่เป็น interactive app หน้า marketing, หน้า "about", หน้า landing ของ deals — ส่วนใหญ่เป็น **content**: ต้องเร็ว, ให้ search engine index ได้, และ ship น้ำหนักเบา SPA คือเครื่องมือที่ผิดสำหรับงานนั้น ดังนั้น content slice ของ Mosaic เป็น [Astro](https://astro.build/) app — server-rendered หรือ static เต็มตัว, HTML-first, มี JavaScript เฉพาะที่จำเป็นจริง ๆ

บทนี้สร้าง app นั้นบน port **5003**: config ของ app, หน้าจริงสองสามหน้า, และ layout ที่ share กัน บทถัดไป [Composing SSR →](/mosaic/th/content/composing-ssr/) ตอบคำถามที่ยากกว่า — สิ่งที่ *ไม่ใช่* Module Federation remote จะเข้าร่วมการประกอบได้ยังไง
